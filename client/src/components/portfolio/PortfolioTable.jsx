						...
						return (
							<tr key={idx}>
								...
								<td className='first-td'>
									...
									<Link className='coin-link' to={`/browse/${asset.uuid}`}>{asset.name}</Link>
								</td>
								...
								<td className='profit-loss-col' style={asset.profitLossUnf < 0 ? {color: '#f44336'} : {color: 'hsl(145, 100%, 36%)'}}>
									...
									<div className='profit-loss'>
										...
										{(asset.profitLossPercent !== undefined ? asset.profitLossPercent : formatData.formatPercent(Math.abs(formatData.calculatePercentChange(asset.holdings, asset.initialHoldings)))) + '%'}
									</div>
								</td>
								...
								<td className='last-td'>
									<div className='actions'>
										<button className='edit-transaction' onClick={() => showTransactions(asset)}>
											<IoIosAddCircle className='edit-transaction-icon'/>
										</button>
										<button className='delete-asset' onClick={() => showDeleteAssetForm(asset)}>
